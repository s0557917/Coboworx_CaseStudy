...
$(document).ready(() => {
  ...
  const temperatureAverageDiv = document.getElementById('temperature-average');
  ...
  const temperatureAverageText = temperatureAverageDiv.getElementsByTagName('h3')[0];
  ...
  webSocket.onmessage = function onMessage(message) {
    try {
      ...
      if (messageData.LatestTemperature && messageData.TemperatureAverage) {
        const currentTemperature = parseFloat(messageData.LatestTemperature);
