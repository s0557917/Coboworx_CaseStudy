...
const {
  Connection, ReceiverEvents, isAmqpError, parseConnectionString
...
function generateSasToken(resourceUri, signingKey, policyName, expiresInMins) {
  const modifiedResourceUri = encodeURIComponent(resourceUri);

  const expiresInSeconds = Math.ceil(Date.now() / 1000 + expiresInMins * 60);
  const toSign = `${modifiedResourceUri}\n${expiresInSeconds}`;
  ...
  return `SharedAccessSignature sr=${modifiedResourceUri}&sig=${base64UriEncoded}&se=${expiresInSeconds}&skn=${policyName}`;
}
